...
def plot_segstart_model(predictor, segidx, highlight = None, segcount = predictor.default_num_segments):
    plt.scatter([t.insize for t in predictor.instances], [ [i for i in itertools.accumulate(starts)][segidx] for (starts,peaks,_) in predictor.segments ])
    plt.plot([predictor.instances[0].insize, predictor.instances[-1].insize], predictor.endpred[segidx].predict([[predictor.instances[0].insize], [predictor.instances[-1].insize]]), color="green")
    if highlight != None:
        plt.scatter([highlight.insize], [[i for i in itertools.accumulate(predictor.get_segments(highlight.mem, segcount)[0])][segidx]], color = "red")
    plt.show()
...
def plot_peak_model(predictor, segidx, highlight = None, segcount = predictor.default_num_segments):
    ...
    plt.plot([predictor.instances[0].insize, predictor.instances[-1].insize], predictor.peakpred[segidx].predict([[predictor.instances[0].insize], [predictor.instances[-1].insize]]), color="green")
    if highlight != None:
...
def simulate_workflow(wfdir, numsegs = predictor.default_num_segments, trainprecenteges = [0.25, 0.5, 0.75], seeds = range(10)):
    print('Reading trace data...', file=sys.stderr)

    tasks = dict([ (taskname, Task(taskname, f'{wfdir}/{taskname}')) for taskname in os.listdir(f'{wfdir}/') if os.path.isdir(f'{wfdir}/{taskname}') ])
    predictors = dict([
        ("Dynamic", predictor.Predictor),
        ("Static", predictor.Unipredictor),
        ("Static Partial", predictor.UnipredictorPartial),
        ("Nils", predictor.Unipredictor2),
        ("Nils Partial", predictor.Unipredictor2Partial),
    ...
    print('-----------------------------------', file=sys.stderr)
    print('task_name, train_rate, seed, method, num_train_instances, num_test_instances, task_executions, memory_wastage, execution_time')

    simulations = len(predictors) * len(trainprecenteges) * len(tasks) * len(seeds)#sum(len(t.instances) for t in tasks.values())
    cursimulation = 0
    res = dict()
        ...
        for perc in trainprecenteges:
            ...
            for s in seeds:
                for (pname,(e,w,t)) in simulate_task(task, predictors, perc, s, numsegs).items():
                    ...
                    print(f'{name}, {perc}, {s}, {pname}, {num_train}, {len(task.instances) - num_train}, {e}, {w}, {t}')
                    print(f'{cursimulation}/{simulations}', file=sys.stderr, end='\r')
            for pname in execs:

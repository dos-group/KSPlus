...
class Predictor:
    def __init__(self, tasks, num_segments = default_num_segments):
        ...
        self.endpred = [ LinearRegression().fit(np.array([t.insize for t in tasks]).reshape(-1,1), [se[i] for se in segends]) for i in range(num_segments) ]
        self.peakpred = [ LinearRegression().fit(np.array([t.insize for t in tasks]).reshape(-1,1), [sp[i] for sp in segpeaks]) for i in range(num_segments) ]

    def get_model(self, taskinstance):
        return model.Model([r.predict([[taskinstance.insize]])[0] * 0.85 for r in self.endpred], [r.predict([[taskinstance.insize]])[0] * 1.1 for r in self.peakpred])
...
class Unipredictor:
    def __init__(self, tasks, num_segments = default_num_segments):
        ...
        segpeaks = [ [max(t.mem[start:end]) for start,end in zip(itertools.chain([0],segends[i]), segends[i])] for i,t in enumerate(tasks)]
        self.timepred = LinearRegression().fit(np.array([t.insize for t in tasks]).reshape(-1,1), [se[-1] for se in segends])
        self.peakpred = [ LinearRegression().fit(np.array([t.insize for t in tasks]).reshape(-1,1), [sp[i] for sp in segpeaks]) for i in range(num_segments) ]
        self.offsets = [max([segpeaks[tidx][sidx] - self.peakpred[sidx].predict(np.array([self.instances[tidx].insize]).reshape((-1, 1)))[0] for tidx in range(len(segpeaks))]) for sidx in range(len(self.peakpred))]

    def get_model(self, taskinstance):
        time = self.timepred.predict([[taskinstance.insize]])[0]
        return model.Unimodel([int(float(time) / self.num_segments) * i for i in range(1,self.num_segments+1)], [r.predict([[taskinstance.insize]])[0] + off for r,off in zip(self.peakpred, self.offsets)])
...
class UnipredictorPartial(Unipredictor):
    def get_model(self, taskinstance):
        time = self.timepred.predict([[taskinstance.insize]])[0]
        return model.UnimodelPartial([int(float(time) / self.num_segments) * i for i in range(1,self.num_segments+1)], [r.predict([[taskinstance.insize]])[0] + off for r,off in zip(self.peakpred, self.offsets)])
